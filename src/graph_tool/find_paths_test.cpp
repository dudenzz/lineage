...
public:
    ...
    void load_test_data(const std::string& filename) {
        ...
        while (file >> neg >> s >> o >> r) {
            ...
            test_triples.push_back({s, r, o, neg});
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    Graph g;
    std::ifstream infile("experiment");
    std::string experiment_name;
    infile >> experiment_name;
    infile.close();
    g.load_from_file("../../csv/kgs/" + experiment_name + "/triples.data");
    g.load_test_data("../../csv/kgs/" + experiment_name + "/test.triples.data");
    
    std::ofstream outfile("../../csv/kgs/" + experiment_name + "/test_paths_output.txt");
    std::atomic<long> processed(0);
    int total = g.test_triples.size();
    ...
    {   
        ...
        for (int i = 0; i < total; ++i) {
            const Triple& t = g.test_triples[i];
            local_found.clear();
